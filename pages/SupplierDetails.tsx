...
import { 
  ...
  Clock, ArrowRight, UserCircle2, Loader2, Share2
} from 'lucide-react';
            ...
            <div className="flex-1 pt-4 md:pt-16">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-sm text-slate-600">
                    ...
                    <span className="flex items-center gap-1">
                      <MapPin size={16} className="text-slate-400" /> {supplier.address || supplier.city}
                    </span>
                ...
                <div className="flex gap-3">
                   <button className="flex-1 md:flex-none bg-primary-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-primary-700 shadow-lg shadow-primary-500/20 flex items-center justify-center gap-2 transition-transform active:scale-95">
                      <Phone size={18} /> إتصل الآن
                   </button>
                   <button className="bg-white border border-slate-200 text-slate-700 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                      <Share2 size={20} />
                   </button>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
              <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                 ...
                 <button className="text-primary-600 font-bold text-sm hover:bg-primary-50 px-3 py-2 rounded-lg transition-colors">
                   + أضف تقييمك
                 </button>
              </div>
                  ...
                  {supplier.reviews.map(review => (
                    <div key={review.id} className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-slate-600 text-sm mt-2 bg-slate-50 p-3 rounded-lg rounded-tr-none">
                          {review.comment}
                        </p>
                      </div>
                    </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
               <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                 <MapPin size={18} className="text-primary-600" /> الموقع على الخريطة
               </h3>
               <div className="w-full h-64 bg-slate-100 rounded-xl overflow-hidden relative">
                 <iframe 
                   ...
                   marginHeight={0} 
                   marginWidth={0} 
                   src={`https://maps.google.com/maps?q=${encodeURIComponent(supplier.address || supplier.city)}&t=&z=13&ie=UTF8&iwloc=&output=embed`}
                 ...
                 <a 
                   ...
                   target="_blank" 
                   rel="noreferrer"
                   className="absolute bottom-3 right-3 bg-white text-slate-800 text-xs font-bold px-3 py-2 rounded shadow-md hover:bg-primary-50"
                 >
                   فتح في Google Maps
                 </a>
               </div>
